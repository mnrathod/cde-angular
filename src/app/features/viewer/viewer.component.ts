...
@Component({
  ...
  template: `
    ...
    >
      <!-- Top bar (Asite navy) -->
      <div
        class="flex items-center h-11 px-3 gap-2 flex-shrink-0 text-white"
      ...
      <!-- Markup toolbar -->
      <div
        class="flex items-center px-3 py-1 gap-1 flex-shrink-0 flex-wrap border-b"
      ...
      <div class="flex flex-1 overflow-hidden">
        ...
        <div
          #viewerContainer
          class="flex-1 overflow-auto relative flex items-start justify-center p-4"
        ...
        >
          ...
          @if (errorMsg()) {
            <div
              class="max-w-md mx-auto mt-16 p-6 bg-red-900/30 rounded-lg border border-red-500/30 text-red-300 text-sm"
            >
              ⚠️ {{ errorMsg() }}
            </div>
          }
        ...
        <!-- Annotation sidebar -->
        <div
          class="w-64 bg-white border-l border-gray-200 flex flex-col flex-shrink-0"
        >
          <div
            class="p-3 border-b border-gray-200 font-semibold text-sm text-gray-700"
          >
          ...
          <div class="flex-1 overflow-y-auto p-2 space-y-2">
            @for (ann of annotations(); track ann.id) {
              <div
                class="bg-gray-50 rounded border-l-3 border-accent p-2 text-xs"
...
export class ViewerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.service
      .getAnnotations(id)
      .subscribe((anns) => this.annotations.set(anns));
  }
