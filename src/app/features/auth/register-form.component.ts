  ...
  template: `
    <form (ngSubmit)="submit()" class="space-y-4">
      ...
      <div>
        ...
        <input id="register-email" [(ngModel)]="email" name="email" type="email" required
          autocomplete="email"
          class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
      </div>
      <div>
        ...
        <input id="register-password" [(ngModel)]="password" name="password" type="password" required
          ...
          class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
        <p class="text-xs text-gray-500 mt-1">At least {{ minPasswordLength }} characters.</p>
      </div>
      ...
      <fieldset class="border border-gray-200 rounded p-3">
        <legend class="text-xs font-medium text-gray-600 px-1">Organisation</legend>

        <div class="flex items-start gap-2 mb-2">
          <input id="join-new" type="radio" name="joining" value="new" class="mt-1"
            [checked]="!joiningExisting()" (change)="chooseNewOrganisation()" />
        ...
        @if (!joiningExisting()) {
          <div class="ml-6 mb-3">
            <label for="organisation-name" class="block text-xs font-medium text-gray-600 mb-1">
              Organisation name <span class="text-gray-400">(optional)</span>
            </label>
            <input id="organisation-name" [(ngModel)]="organisationName" name="organisationName" type="text"
              class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
            <p class="text-xs text-gray-500 mt-1">Leave blank and we'll name it after you — you can change it later.</p>
          </div>
        }

        <div class="flex items-start gap-2">
          <input id="join-existing" type="radio" name="joining" value="existing" class="mt-1"
            [checked]="joiningExisting()" (change)="chooseExistingOrganisation()" />
        ...
        @if (joiningExisting()) {
          <div class="ml-6 mt-2">
            <label for="invitation-token" class="block text-xs font-medium text-gray-600 mb-1">Invitation code</label>
            <!--
            ...
            -->
            <input id="invitation-token" [(ngModel)]="invitationToken" name="invitationToken" type="text"
...
export class RegisterFormComponent {
  ...
  submit() {
    ...
    if (this.password.length < MIN_PASSWORD_LENGTH) {
      ...
      this.failed.emit(`Password must be at least ${MIN_PASSWORD_LENGTH} characters.`);
      return;
    }
    if (this.joiningExisting() && !this.invitationToken.trim()) {
      this.failed.emit('Enter the invitation code, or choose to create a new organisation.');
      return;
    }
    ...
    this.auth.register({
      ...
      error: (err: unknown) => {
        ...
        this.failed.emit(problemDetail(
          err, 'Could not create the account. Please try again.'));
      }
    });
  }
}
